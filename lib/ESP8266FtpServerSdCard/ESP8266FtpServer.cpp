...
void FtpServer::clientConnected()
{
  ...
  client.println( "220---   By David Paiva - changed by Ivan R. Cardoso ---");
  client.println( "220 --   Versao "+ String(FTP_SERVER_VERSION) +"   --");
  iCL = 0;
}
...
boolean FtpServer::processCommand()
{
  ...
  else if( ! strcmp( command, "LIST" ))
  {
    ...
    else
    {
     ...
     // else
      if(!dir.isDirectory())
        data.println( "550 Can't open directory " + String(cwdName) );
      else{
        // try.. while( dir.next())
        while(1)
		{
  ...
  else if( ! strcmp( command, "STOR" ))
  {
    ...
    file = SD.open(path, FILE_WRITE);
      ...
      {
        // #ifdef FTP_DEBUG
          // Serial.printlprintlnn( "Receiving " +String(parameters));
          // Serial.( "buf ----------------->");
          // Serial.println( buf);
          // Serial.println( "----------------------------- ");
        // #endif
...
boolean FtpServer::doStore()
{
  ...
  int navail = data.available();// Avoid blocking by never reading more bytes than are available

  if (navail > 0)
  {
    
    if (navail > FTP_BUF_SIZE) navail = FTP_BUF_SIZE;// And be sure not to overflow buf.
    
    int16_t nb = data.read((uint8_t*) buf, navail );// int16_t nb = data.readBytes((uint8_t*) buf, FTP_BUF_SIZE );
    ...
    if( nb > 0 )
    {
      ...
      file.write((uint8_t*) buf, nb );// Serial.println( millis() << " " << nb << endl;
      bytesTransfered += nb;
    }
  }
  ...
  //     Serial.println( buf) ;
      // File dataFile;
      // dataFile = SD.open("/att.txt", FILE_WRITE); 
      // dataFile.println(buf);
      // dataFile.write((uint8_t*) buf, nb );
      // dataFile.close();  
  //     file.write((uint8_t*) buf, nb );// Serial.println( millis() << " " << nb << endl;
  //     bytesTransfered += nb;
  //   }
